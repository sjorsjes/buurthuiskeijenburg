...
const format = (year: string | number, month: string | number, day = '01') => {
    return new Date(`${year}-${month}-${day}`).toLocaleString('nl-NL', { month: 'long', year: 'numeric' });
}
...
const filterItems = (data: MarkdownInstance<Record<string, any>>[], m: number, y: number) => {
...
export default (data: MarkdownInstance<Record<string, any>>[]) => {
    ...
    const { month: nextMonth, year: nextYear } = nextMonthAndYear(currentMonth, currentYear);
    const { month: secondNextMonth, year: secondNextYear } = nextMonthAndYear(nextMonth, nextYear);

    return [
        { date: format(currentYear, currentMonth), data: filterItems(data, currentMonth, currentYear)},
        { date: format(nextYear, nextMonth), data: filterItems(data, nextMonth, nextYear)},
        { date: format(secondNextYear, secondNextMonth), data: filterItems(data, secondNextMonth, secondNextYear)}
    ];
}
